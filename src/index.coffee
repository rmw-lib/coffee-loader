...
extensionsRegex = /\.coffee$/

IGNORE = new Set("coffee yaml txt js mjs wasm json node".split(" "))

export resolve = (specifier, context, defaultResolve) =>
  { parentURL = baseURL } = context

  ext = specifier.slice(specifier.lastIndexOf(".") + 1)
  if !IGNORE.has(ext) and !specifier.endsWith(".") and specifier.startsWith(".")
